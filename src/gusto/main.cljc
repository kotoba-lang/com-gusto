(ns gusto.main
  "Kotodama WASM entrypoint for the Gusto clean-room actor (L4) — Clojure port.
  ...
  facts (`gusto.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Employee"   :plural "employees"   :id-prefix "gusto_emp"
    :fields [:uuid :firstName :lastName :email :onboardingStatus :paymentMethod :terminated :flsaStatus] :required [:uuid :firstName]
    :coerce {:terminated :bool}                                            :refs {}}
   {:entity "Company"      :plural "companies"      :id-prefix "gusto_com"
    :fields [:uuid :name :status :entityType :tier]                        :required [:uuid :name]
    :coerce {}                                                              :refs {}}
   {:entity "Compensation" :plural "compensations" :id-prefix "gusto_cmp"
    :fields [:rate :paymentUnit :flsaStatus :effectiveDate :adjustForMinimumWage] :required [:rate :paymentUnit]
    :coerce {:adjustForMinimumWage :bool}                                  :refs {}}
   {:entity "Job"        :plural "jobs"        :id-prefix "gusto_job"
    :fields [:uuid :title :rate :paymentUnit :primary]                     :required [:uuid :title]
    :coerce {:primary :bool}                                               :refs {}}
   {:entity "Contractor" :plural "contractors" :id-prefix "gusto_con"
    :fields [:firstName :lastName :businessName :wageType :contractorType :isActive] :required [:firstName :lastName]
    :coerce {:isActive :bool}                                              :refs {}}
   {:entity "Payroll"    :plural "payrolls"    :id-prefix "gusto_pay"
    :fields [:uuid :processed :offCycle :offCycleReason]                   :required [:uuid :processed]
    :coerce {:processed :bool :offCycle :bool}                             :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"gusto.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "gusto-compat" :tier tier :entities entities} 200])
